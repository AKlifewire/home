...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    if (!('deviceId' in deviceConfig) || typeof deviceConfig.deviceId !== 'string') {
      validationErrors.push('deviceId (string)');
    }
    if (!('deviceType' in deviceConfig) || typeof deviceConfig.deviceType !== 'string') {
      validationErrors.push('deviceType (string)');
    }
    if (!('components' in deviceConfig) || !Array.isArray(deviceConfig.components)) {
      validationErrors.push('components (array)');
    }
    if (!('metadata' in deviceConfig) || typeof deviceConfig.metadata !== 'object') {
      validationErrors.push('metadata (object)');
    }
