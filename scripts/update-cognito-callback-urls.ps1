# Update Cognito User Pool Client callback URLs with actual Amplify domain
param(
    [string]$EnvName = "dev"
)

Write-Host "Updating Cognito callback URLs for environment: $EnvName"
...
Write-Host "Client ID: $USER_POOL_CLIENT_ID"
...
Write-Host "Cognito callback URLs updated successfully!"